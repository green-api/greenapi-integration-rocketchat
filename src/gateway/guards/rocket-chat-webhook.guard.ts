...
export class RocketChatWebhookGuard implements CanActivate {
	...
	async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		if (!token) {
			this.logger.warn("X-Rocketchat-Livechat-Token header is missing", {
				body: request.body,
		...
		if (!message.messages?.[0]?.agentId) {
			this.logger.warn("Not an agent message", {
				body: request.body,
				headers: request.headers,
			});
		...
		if (!agentEmail) {
			this.logger.warn("Invalid webhook format", {
				body: request.body,
				headers: request.headers,
			});
		...
		if (!user) {
			this.logger.warn("No user with such credentials", {
				body: request.body,
				headers: request.headers,
			});
		...
		if (!instance) {
			this.logger.warn("Instance by phone number is not found or the instance unexpectedly changed its state", {
				body: request.body,
				headers: request.headers,
			});
			throw new HttpException({message: "Instance by phone number is not found or the instance unexpectedly changed its state"}, HttpStatus.OK);
		}

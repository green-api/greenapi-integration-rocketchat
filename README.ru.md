...
Примечание: Для регистрации рабочего пространства (`/greenapi.register-workspace`) пользователь должен иметь права
администратора или разрешение `view-livechat-manager`. Для регистрации обычного агента (`/greenapi.register-user`)
достаточно стандартных прав агента.
...
### 1. Зарегистрируйте ваше рабочее пространство в адаптере:

```
/greenapi.register-workspace [rocket-chat-id] [rocket-chat-token]
```

- `rocket-chat-id`: Ваш ID в Rocket.Chat (с правами администратора или разрешением view-livechat-manager)
- `rocket-chat-token`: Ваш персональный API токен Rocket.Chat

В ответе вы получите командный токен рабочего пространства. **Вам необходимо вставить этот токен в настройках приложения
GREEN-API:**
...
4. Вставьте токен вашего рабочего пространства
...
Для выполнения этой команды требуется предварительная регистрация рабочего пространства и установка командного токена
рабочего пространства в настройках приложения.
